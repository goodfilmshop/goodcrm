...
export function createHandler({ getEnv, persist, resolveDestination, enrich = async () => {}, background = () => {} }) {
  return async request => {
    const secret = getEnv('LINE_GFS_CHANNEL_SECRET');
    let destination = getEnv('LINE_GFS_BOT_USER_ID');
    if (!destination && resolveDestination && secret && getEnv('LINE_GFS_INTAKE_ENABLED') === 'true') {
      try { destination = await resolveDestination(); } catch { destination = undefined; }
    }
    const configured = getEnv('LINE_GFS_INTAKE_ENABLED') === 'true' && Boolean(secret) && /^U[0-9a-f]{32}$/.test(destination || '');
    if (request.method === 'GET' && new URL(request.url).pathname.endsWith('/health')) return reply(200, {configured, account:'@249izgyn', ...(configured ? {botUserId:destination} : {})});
    ...
    if (!await verifySignature(raw,request.headers.get('x-line-signature'),secret)) return reply(401,{error:'Invalid signature'});
    let events;
    try {

...
export function createHandler({ getEnv, persist, resolveDestination, enrich = async () => {}, background = () => {} }) {
  return async request => {
    const secret = getEnv('LINE_MHL_BRIDGE_SECRET');
    let destination = getEnv('LINE_MHL_BOT_USER_ID');
    if (!destination && resolveDestination && secret && getEnv('LINE_MHL_INTAKE_ENABLED') === 'true') {
      try { destination = await resolveDestination(); } catch { destination = undefined; }
    }
    const configured = getEnv('LINE_MHL_INTAKE_ENABLED') === 'true' && Boolean(secret) && /^U[0-9a-f]{32}$/.test(destination || '');
    if (request.method === 'GET' && new URL(request.url).pathname.endsWith('/health')) return reply(200, {configured, account:'@320opqkc', ...(configured ? {botUserId:destination} : {})});

-- Two GFS accounts; existing per-account deduplication and admin permissions retained.
set local lock_timeout='5s';
alter table public.line_intake_contacts drop constraint line_intake_contacts_account_key_check, add constraint line_intake_contacts_account_key_check check(account_key in ('gfs-line-249izgyn','gfs-line-095jvuls'));
...
begin
  if p_account is null or p_account not in ('gfs-line-249izgyn','gfs-line-095jvuls') or jsonb_typeof(p_events) <> 'array' then raise exception 'Invalid intake'; end if;
...
begin
  ...
  if p_decision='dismiss' then
    ...
    for attempt in 1..5 loop
      ...
      values(legacy,trim(p_name),now(),'LINE',trim(p_name),case item.account_key when 'gfs-line-095jvuls' then 'GFS LINE OA @095jvuls / @goodfilm' else 'GFS LINE OA @249izgyn / @3mfilm' end,auth.uid(),auth.uid())
